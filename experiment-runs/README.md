...
All arrays are explicit, and nested objects reject unsupported fields. Status values, calibration values, and every reference id are matched case-sensitively, so casing drift cannot introduce a second spelling of a canonical value. Calibration is limited to `known`, `unknown`, and `not-applicable`: a Boolean was rejected because it cannot distinguish absent calibration knowledge from a method that does not require calibration. Measurement values are numeric; estimates and ranges belong in uncertainty/limitations rather than fake decimal precision.
