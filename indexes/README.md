...
Experiment definitions and completed runs remain distinct. A linked definition counts as experiment coverage; only a run whose canonical status is `completed` counts as performed evidence. Planned runs therefore produce `partial` run-evidence coverage, not performed evidence. Canonical files remain authoritative, and humans decide whether any candidate warrants research.
