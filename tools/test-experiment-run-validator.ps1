...
try {
    ...
    Expect-Failure 'index falsifies run status' { Replace-Text $tempIndex '- `near-frequency-beating-planned-a` — `planned` — not performed' '- `near-frequency-beating-planned-a` — `completed` — not performed' }
    Expect-Failure 'index invents a run date' { Replace-Text $tempIndex '- `near-frequency-beating-planned-a` — `planned` — not performed' '- `near-frequency-beating-planned-a` — `planned` — 2026-08-18' }
    Expect-Failure 'index misattributes a run to another experiment' { Replace-Text $tempIndex '- `near-frequency-beating-planned-a` — `near-frequency-beating`' '- `near-frequency-beating-planned-a` — `waveform-timbre-comparison`' }
    Expect-Failure 'index leaks raw object text' { Replace-Text $tempIndex '- `near-frequency-beating-planned-a` — `planned` — not performed' '- `near-frequency-beating-planned-a` — `planned` — System.Collections.Hashtable' }
    Expect-Failure 'status enum case drift' { Replace-Text (Join-Path $tempRuns 'near-frequency-beating-planned-a.yaml') 'status: "planned"' 'status: "Planned"' }
    Expect-Failure 'source reference case drift' { Replace-Text (Join-Path $tempRuns 'near-frequency-beating-planned-a.yaml') 'source_refs: []' 'source_refs: ["SESSION-01-WHAT-IS-SOUND"]' }
    Expect-Failure 'calibration enum case drift' { $path=Join-Path $tempRuns 'near-frequency-beating-planned-a.yaml'; Replace-Text $path 'run_date: null' 'run_date: "2026-08-18"'; Replace-Text $path 'status: "planned"' 'status: "completed"'; Replace-Text $path 'measurements: []' 'measurements: [{"quantity":"frequency","value":441.8,"unit":"Hz","method":"FFT peak","tool":"analyzer","calibration":"Known","uncertainty":null,"limitations":[]}]'; & $builder -RunDirectory $tempRuns -OutputPath $tempIndex | Out-Null }
    Expect-Failure 'run dated in the future' { $path=Join-Path $tempRuns 'near-frequency-beating-planned-a.yaml'; Replace-Text $path 'run_date: null' 'run_date: "2999-12-31"'; Replace-Text $path 'status: "planned"' 'status: "completed"'; Replace-Text $path 'observations: []' 'observations: [{"statement":"beating was audible","context":"440 Hz and 442 Hz, same headphones"}]'; & $builder -RunDirectory $tempRuns -OutputPath $tempIndex | Out-Null }
