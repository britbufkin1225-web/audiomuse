...
$ids=@{}; foreach($record in $records){ if($ids.ContainsKey($record.id)){throw "Duplicate experiment id: $($record.id)"}; $ids[$record.id]=$true }
...
$vocabIds=@{}; Get-ChildItem (Join-Path $repoRoot 'vocabulary/entries') -Filter '*.yaml' -File | ForEach-Object { [regex]::Matches([IO.File]::ReadAllText($_.FullName),'(?m)^id: "(?<id>[a-z0-9-]+)"\r?$') | ForEach-Object {$vocabIds[$_.Groups['id'].Value]=$true} }
...
foreach($record in $records){
    if($record.status -notin $validStatus){throw "Invalid experiment status for $($record.id): $($record.status)"}; if($record.type -notin $validType){throw "Invalid experiment type for $($record.id): $($record.type)"}; if($record.difficulty -notin $validDifficulty){throw "Invalid experiment difficulty for $($record.id): $($record.difficulty)"}
    ...
    foreach($ref in $record.node_refs){if(-not $nodeIds.ContainsKey($ref)){throw "Unresolved node reference for $($record.id): $ref"};$nodeRefs++}
    foreach($ref in $record.vocabulary_refs){if(-not $vocabIds.ContainsKey($ref)){throw "Unresolved vocabulary reference for $($record.id): $ref"};$vocabRefs++}
    foreach($ref in $record.session_refs){if(-not $sourceTypes.ContainsKey($ref) -or $sourceTypes[$ref] -ne 'session'){throw "Invalid session reference for $($record.id): $ref"};$sessionRefs++}
