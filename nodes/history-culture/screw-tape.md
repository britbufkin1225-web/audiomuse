...
The performance and the master are the same event. Everything below them is a copy, and every copy is a generation removed from the audio the room produced. That is why AudioMuse treats format as historically significant rather than incidental: the recording is a document of a single unrepeatable pass, and most listeners heard a duplicate of it.
...
What the sources do not establish is the duplication mechanics. No retrieved source in this phase describes the deck, the dubbing speed, the tape stock, or whether copies were run one at a time or in banks. `cassette-duplication` therefore holds what is generally true of the format and explicitly does not attribute a specific method to Screw. This is the single largest technical gap left open by Phase 12C, and it is a documented gap rather than a silent one.
